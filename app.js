...
function getFlagLeafletMarkerIcon({ fill, stroke, size }) {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 48 48">
      <path d="M15 40V8" stroke="${pole}" stroke-width="3.2" stroke-linecap="round"/>
      <path d="M15 9H39L35 17L39 25H15Z" fill="${fill}" stroke="${stroke}" stroke-width="2.4" stroke-linejoin="round"/>
      <circle cx="15" cy="40" r="4.5" fill="${fill}" stroke="${stroke}" stroke-width="2.2"/>
    </svg>
  `;
...
function getMapMarkerPalette(mapType = state.preferredMapType) {
  if (mapType === "dark") {
    return {
      selectedFill: "#f6b51d",
      selectedStroke: "#090806",
      localFirstFill: "#ffd75a",
      localFill: "#fff0b8",
      localStroke: "#120f08",
      selectedFillRgb: "246, 181, 29",
      selectedStrokeRgb: "255, 238, 174",
      selectedShadowRgb: "255, 190, 46",
      selectedRingRgb: "255, 222, 114",
      selectedShadow: "#ffbe2e"
    };
  }
  ...
  return {
    ...
    localFirstFill: "#7cc7ff",
    localFill: "#eaf7ff",
    localStroke: "#08213b",
    selectedFillRgb: "47, 146, 255",
    selectedStrokeRgb: "220, 248, 255",
    selectedShadowRgb: "47, 170, 255",
    selectedRingRgb: "255, 255, 255",
    selectedShadow: "#4fd9ff"
  };
}
...
function renderLocalSuggestions() {
  ...
  if (!state.userPosition) {
    renderLocalResults(randomPlaces.map((place) => ({ ...place, mode: "Losowo" })));
    return;
  }
  ...
  const randomFill = getRandomLocalPlaces(2, usedNames).map((place) => ({ ...place, mode: "Losowo" }));
...
function getRandomLocalPlaces(count, excludedNames = new Set()) {
  const dateKey = `${state.selectedDate.getFullYear()}-${state.selectedDate.getMonth() + 1}-${state.selectedDate.getDate()}`;
  return localPlaces
